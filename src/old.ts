...
const FIELD_PROPERTIES: FieldProperties[] = [
...
export const expandCronExpression2 = ({
    cronString,
}: {
    cronString: string
}) => {
    const [unparsedFields, command] = validateCronStringParts({ cronString })
    const expandedFields = expandFields({ unparsedFields })
    printFormattedTable({ expandedFields, command })
}

const validateCronStringParts = ({
    cronString,
}: {
    cronString: string
}): [string[], string] => {
...
const expandFields = ({
    ...
    return unparsedFields.map((field, i) => {
        const { name, min, max } = FIELD_PROPERTIES[i]
